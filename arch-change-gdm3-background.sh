...
if ! [[ "$distro" =~ (Arch|Gentoo) ]]; then
    echo 'Sorry, this script only works with Arch or Gentoo distros.'
    exit 1
fi
...
# Compile and move gresoure.
  ...
  # If no file was submited or file submited isn't an image,
    ...
    echo 'Usage: sudo ./arch-change-gdm3-background /path/to/image.*g'
    echo 'Usage: sudo ./arch-change-gdm3-background \#yourhexcode'
